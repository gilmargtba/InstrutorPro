...
export class LeafletMapProvider implements MapProvider {
  ...
  mount(element:HTMLElement,onSelect:(id:string)=>void){this.destroy();this.onSelect=onSelect;this.map=L.map(element,{zoomControl:true}).setView([-14.2,-51.9],4);L.tileLayer('/api/v1/map/tiles/{z}/{x}/{y}.png',{attribution:'&copy; MapTiler &copy; OpenStreetMap contributors',maxZoom:19}).addTo(this.map)}
