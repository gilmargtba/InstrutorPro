...
export abstract class MapProvider { abstract mount(element:HTMLElement,onSelect:(id:string)=>void):void;abstract render(items:SearchInstructor[],selectedId:string|null):void;abstract select(id:string):void;abstract refresh():void;abstract destroy():void; }
...
export class LeafletMapProvider implements MapProvider {
  ...
  render(items:SearchInstructor[],selectedId:string|null){if(!this.map)return;this.markers.forEach(m=>m.remove());this.markers.clear();const group=L.featureGroup();for(const item of items){const initials=item.display_name.split(' ').slice(0,2).map(part=>part.match(/[\p{L}\p{N}]/u)?.[0]??'').join('').toUpperCase()||'IP';const icon=L.divIcon({className:'demo-marker',html:`<span aria-hidden="true">${initials}</span>`,iconSize:[42,42],iconAnchor:[21,21]});const marker=L.marker([item.latitude,item.longitude],{title:item.display_name,icon}).on('click',()=>this.onSelect(item.id));marker.bindTooltip(item.display_name);marker.addTo(this.map);group.addLayer(marker);this.markers.set(item.id,marker)}if(items.length)this.map.fitBounds(group.getBounds().pad(.25),{maxZoom:14});if(selectedId)this.select(selectedId)}
