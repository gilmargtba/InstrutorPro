...
@Component({
  ...
  <form class="onboarding" (ngSubmit)="submit()">
    ...
    <fieldset><legend>Área pública de atendimento</legend><p>Informe apenas a cidade onde atende. Buscaremos automaticamente o centro público aproximado. Não informe sua residência.</p><label>Cidade<input name="city" [(ngModel)]="form.instructor_city" (ngModelChange)="clearPublicLocation()" required></label><label>UF<select name="uf" [(ngModel)]="form.instructor_uf" (ngModelChange)="clearPublicLocation()" required><option value="">Selecione</option>@for(uf of ufs;track uf){<option [value]="uf">{{uf}}</option>}</select></label><button class="button secondary location-search" type="button" (click)="findPublicLocation()" [disabled]="locating()||!form.instructor_city||!form.instructor_uf">{{locating()?'Buscando…':'Buscar centro público da cidade'}}</button>@if(locationMessage()){<p class="location-result" role="status">{{locationMessage()}}</p>}<label>Latitude pública<input name="latitude" [(ngModel)]="form.service_latitude" type="number" readonly required></label><label>Longitude pública<input name="longitude" [(ngModel)]="form.service_longitude" type="number" readonly required></label><label>Raio<select name="radius" [(ngModel)]="form.service_radius_km"><option [ngValue]="5">5 km</option><option [ngValue]="10">10 km</option><option [ngValue]="20">20 km</option><option [ngValue]="50">50 km</option></select></label><label class="check"><input name="locationAuthorization" [(ngModel)]="form.service_location_authorized" type="checkbox" required> Autorizo a publicação desta área aproximada de atendimento.</label></fieldset>
    <button class="button primary" [disabled]="sending()">Concluir cadastro</button>@if(message()){<p role="alert">{{message()}}</p>}
  </form></section>`,
  styles: [`.onboarding{display:grid;gap:1rem}.onboarding label{display:grid;gap:.35rem;font-weight:700}.onboarding input,.onboarding select,.onboarding textarea{padding:.75rem;border:1px solid #bad4d1;border-radius:.7rem}.onboarding input[readonly]{background:#eef5f4;color:#345}.onboarding textarea{min-height:7rem}.onboarding fieldset{display:grid;grid-template-columns:1fr 1fr;gap:1rem;padding:1.2rem;border:1px solid #d6e5e3;border-radius:1rem}.onboarding fieldset p,.onboarding fieldset legend,.check,.location-search,.location-result{grid-column:1/-1}.check{grid-template-columns:auto 1fr!important}@media(max-width:650px){.onboarding fieldset{grid-template-columns:1fr}}`],
})
export class RealInstructorOnboardingComponent {
  ...
  constructor(){this.http.get<any>('/account/me/').subscribe({next:r=>{const instructor=r.instructor;if(!instructor){void this.router.navigateByUrl('/cadastro/instrutor');return}this.category=instructor.categories?.[0]||'B';this.transmission=instructor.transmission_options?.[0]||'MANUAL';this.form={...this.form,instructor_display_name:instructor.display_name||'',bio:instructor.bio||'',whatsapp:instructor.whatsapp||'',price_amount:instructor.price_amount?Number(instructor.price_amount):null,duration_minutes:instructor.duration_minutes??60,instructor_city:instructor.city||'',instructor_uf:instructor.uf||'',service_latitude:instructor.service_latitude??null,service_longitude:instructor.service_longitude??null,service_radius_km:instructor.service_radius_km??10,service_location_authorized:!!instructor.service_location_authorized,vehicle:{category:instructor.vehicle?.category||this.category,make:instructor.vehicle?.make||'',model:instructor.vehicle?.model||'',year:instructor.vehicle?.year||new Date().getFullYear(),transmission:instructor.vehicle?.transmission||this.transmission,...(instructor.vehicle?.ownership_type?{ownership_type:instructor.vehicle.ownership_type}:{})}};if(this.form.service_latitude!==null&&this.form.service_longitude!==null){this.locationMessage.set(`Centro público salvo: ${this.form.instructor_city}/${this.form.instructor_uf}.`)}},error:()=>void this.router.navigateByUrl('/entrar')})}
  clearPublicLocation(){this.form.service_latitude=null;this.form.service_longitude=null;this.locationMessage.set('')}
  findPublicLocation(){if(!this.form.instructor_city||!this.form.instructor_uf)return;this.locating.set(true);this.locationMessage.set('');const expectedUf=String(this.form.instructor_uf).toUpperCase();this.search.geocode(`${this.form.instructor_city}, ${expectedUf}, Brasil`,5).subscribe({next:response=>{this.locating.set(false);const point=response.results.find(item=>item.uf===expectedUf);if(!point){this.clearPublicLocation();this.locationMessage.set('Cidade não encontrada nessa UF. Revise a cidade e tente novamente.');return}this.form.service_latitude=point.latitude;this.form.service_longitude=point.longitude;this.locationMessage.set(`Centro público encontrado: ${point.label}.`)},error:e=>{this.locating.set(false);this.clearPublicLocation();this.locationMessage.set(e?.error?.detail||'Não foi possível buscar a cidade agora. Tente novamente.')}})}
