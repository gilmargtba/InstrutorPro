...
@Component({
  ...
  template: `<section class="page narrow"><div class="demo-flag">DEV/TEST · dados sintéticos</div><header class="page-head"><div><p class="eyebrow">Instrutor</p><h1>Encontre alunos na sua região.</h1><p>Apareça nas buscas, mostre seu perfil profissional e receba contatos pelo WhatsApp.</p></div></header><article class="notice"><h2>Crie seu perfil profissional</h2><p>Cadastro não significa aprovação. As informações serão analisadas e o profissional deve atender aos requisitos aplicáveis.</p><label><input type="checkbox" [(ngModel)]="accepted"> Declaro que as informações fornecidas são verdadeiras e estou ciente de que meu cadastro será analisado antes da publicação.</label><a class="button primary" [class.disabled]="!accepted" [routerLink]="accepted?'/profissional/instrutor/onboarding':null">Criar meu perfil</a></article><div class="entry"><a routerLink="/entrar">Já tenho cadastro — Entrar</a></div></section>`,
...
export class InstructorStatusComponent {
  ...
  constructor(){this.http.get<{instructor?:any}>('/account/me/').subscribe(r=>this.profile.set(r.instructor))}
}

@Component({selector:'app-instructor-portal',imports:[RouterLink],template:`<section class="page wide"><header class="page-head"><div><p class="eyebrow">Seu perfil</p><h1>Olá, {{profile()?.display_name}}</h1><p>{{status(profile()?.publication_status)}}</p></div><a class="button secondary" routerLink="/profissional/instrutor/status">Ver status completo</a></header>@if(profile();as p){<h2>Este mês</h2><div class="portal-grid"><article><h3>Visualizações na busca</h3><strong>{{p.metrics?.search_impressions||0}}</strong></article><article><h3>Visitas ao perfil</h3><strong>{{p.metrics?.profile_views||0}}</strong></article><article><h3>Contatos pelo WhatsApp</h3><strong>{{p.metrics?.whatsapp_clicks||0}}</strong><p>Contato iniciado, não significa aula vendida.</p></article><article><h3>Preço da aula</h3><p>{{p.offer?'R$ '+p.offer.price_amount+' · '+p.offer.duration_minutes+' minutos':'Preço ainda não informado.'}}</p></article><article><h3>Veículo</h3><p>{{p.vehicle?p.vehicle.make+' '+p.vehicle.model:'Nenhum veículo cadastrado.'}}</p></article><article><h3>Atende em</h3><p>{{p.service_area?p.service_area.city+'/'+p.service_area.uf+' · '+p.service_area.radius_km+' km':'Área não cadastrada.'}}</p></article></div>}@else{<p>Carregando seu painel…</p>}</section>`,styles:[`.portal-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem}.portal-grid article{padding:1.5rem;border:1px solid #d6e5e3;border-radius:1rem;background:#fff}.portal-grid strong{font-size:2rem;color:#0c807e}@media(max-width:800px){.portal-grid{grid-template-columns:1fr}}`]})
export class InstructorPortalComponent{private http=inject(HttpClient);profile=signal<any>(undefined);constructor(){this.http.get<{instructor?:any}>('/demo/marketplace/session/me/').subscribe(r=>this.profile.set(r.instructor))}status(value:string){return value==='APPROVED'?'Publicado':value==='SUSPENDED'?'Suspenso':'Em análise'}}
