...
@Component({
  ...
  template: `<section class="page narrow"><div class="demo-flag">DEMO · contas sintéticas</div><header class="page-head"><div><p class="eyebrow">Acesso</p><h1>Entrar no InstrutorPro</h1></div></header><form class="login" (ngSubmit)="submit()"><label>E-mail<input type="email" name="email" [(ngModel)]="email" required></label><label>Senha<input type="password" name="password" [(ngModel)]="password" required></label><button class="button primary" [disabled]="sending">Entrar</button><a href="#" (click)="$event.preventDefault()">Esqueci minha senha</a><a routerLink="/aluno">Criar conta</a>@if(error){<p>{{error}}</p>}</form></section>`,
